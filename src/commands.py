...
def cmdList(cmdWork):
  ...
  for lineNumber in data.index:
    if lineNumber in range (first, last + 1):
      strWork = strWork + data.codeList[lineNumber]
      if lineNumber < last:
        strWork = strWork + '\n'
...
def cmdOld(cmdWork):
  ...
  cmdNew(cmdWork)
  return readWrite.loadCodeFile(fileName)
...
def cmdSave(cmdWork):
  ...
  return readWrite.saveCodeFile(fileName)

# list all basic files in flash memory

def cmdFiles(cmdWork):
  return readWrite.listCodeFiles()
...
def cmdDelete(cmdWork):
  ...
  return readWrite.deleteCodeFile(fileName)
